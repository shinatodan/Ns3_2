...
#define PGPSR_LS_RLS 1

NS_LOG_COMPONENT_DEFINE ("PGpsrRoutingProtocol");
...
// TODO read ns3 api
        ...
        {
                static TypeId tid = TypeId ("ns3::pgpsr::DeferredRouteOutputTag").SetParent<Tag> (); //设置parent的tid,获取<Tag>是模版
                return tid;
        }
...
#define PGPSR_MAXJITTER          (HelloInterval.GetSeconds () / 2)
/// Random number between [(-PGPSR_MAXJITTER)-PGPSR_MAXJITTER] used to jitter HELLO packet transmission.
#define JITTER (Seconds (x->GetValue (-PGPSR_MAXJITTER, PGPSR_MAXJITTER)))
#define FIRST_JITTER (Seconds (x->GetValue (0, PGPSR_MAXJITTER))) //first Hello can not be in the past, used only on SetIpv4



NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for PGPSR control traffic, not defined by IANA yet
const uint32_t RoutingProtocol::PGPSR_PORT = 666;
...
{
        static TypeId tid = TypeId ("ns3::pgpsr::RoutingProtocol")
                            ...
                            .AddAttribute ("LocationServiceName", "Indicates wich Location Service is enabled",
                                           EnumValue (PGPSR_LS_GOD),
                                           MakeEnumAccessor (&RoutingProtocol::LocationServiceName),
                                           MakeEnumChecker (PGPSR_LS_GOD, "GOD",
                                                            PGPSR_LS_RLS, "RLS"))
                                  ...
                                  LocalDeliverCallback lcb, ErrorCallback ecb)
{
        ...
        {
                NS_LOG_LOGIC ("No pgpsr interfaces");
                return false;
        }
        NS_ASSERT (m_ipv4 != 0);
        NS_ASSERT (p != 0);
        ...
        // TypeHeader tHeader (PGPSRTYPE_POS);
        // //去掉packet的包头
        ...
        if (m_ipv4->IsDestinationAddress (dst, iif))
        {
                ...
                if (!tHeader.IsValid ())
                {
                        NS_LOG_DEBUG ("PGPSR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
                        NS_LOG_DEBUG ("RoutingInput meet error");
                        return false;
                }
                NS_LOG_DEBUG ("Received packet");
                //如果是POS的包，就把POS的包头再去掉，所以不需要了解pos的信息了，直接去掉）
                if (tHeader.Get () == PGPSRTYPE_POS)
                {
                        PositionHeader phdr;
...
/*        
        //宛先の未来位置
        Vector f_dist,dstvel;
        dstvel = m_locationService->GetVelocity(dst);
        f_dist.x = dstvel.x + dstPos.x;
        f_dist.y = dstvel.y + dstPos.y;
*/        
        ...
        else
        {
                ...
                //nextHop = m_neighbors.BestNeighbor (f_dist, myPos);
        }
        ...
        if (nextHop != Ipv4Address::GetZero ())
        {
                ...
                NS_LOG_DEBUG ("Now time is "<<time_now);
                NS_LOG_DEBUG ("In RouteOutput Nexthop is  "<<nextHop);
                //NS_LOG_DEBUG ("Exist route to " << route->GetDestination () << " from interface " << route->GetSource ());
                if (oif != 0 && route->GetOutputDevice () != oif)
                {
                        NS_LOG_DEBUG ("Output device doesn't match. Dropped.");
...
RoutingProtocol::SendPacketFromQueue (Ipv4Address dst)
{
        ...
        if(recovery)
        {
                ...
                while(m_queue.Dequeue (dst, queueEntry))
                {
                        ...
                        if (!tHeader.IsValid ())
                        {
                                NS_LOG_DEBUG ("PGPSR message " << p->GetUid () << " with unknown type received: " << tHeader.Get () << ". Drop");
...
RoutingProtocol::NotifyInterfaceUp (uint32_t interface)
{
        ...
        {
                NS_LOG_WARN ("PGPSR does not work with more then one address per each interface.");
        }
        ...
        NS_ASSERT (socket != 0);
        socket->SetRecvCallback (MakeCallback (&RoutingProtocol::RecvPGPSR, this));
        socket->Bind (InetSocketAddress (Ipv4Address::GetAny (), PGPSR_PORT));
...
//接受到socket的包
void
RoutingProtocol::RecvPGPSR (Ptr<Socket> socket)
{
        NS_LOG_FUNCTION (this << socket);
        Address sourceAddress;
        Ptr<Packet> packet = socket->RecvFrom (sourceAddress);
        //RemoveHeader
        // TypeHeader tHeader1 (PGPSRTYPE_HELLO);
        ...
        {
                NS_LOG_DEBUG ("PGPSR message " << packet->GetUid () << " with unknown type received: " << tHeader.Get () << ". Ignored");
                NS_LOG_DEBUG ("RecvPGPSR meet error");
                return;
        }
        ...
        f_Position.x = hdr.GetOriginFPosx ();
        f_Position.y = hdr.GetOriginFPosy ();
        InetSocketAddress inetSourceAddr = InetSocketAddress::ConvertFrom (sourceAddress);
        Ipv4Address sender = inetSourceAddr.GetIpv4 ();
        Ipv4Address receiver = m_socketAddresses[socket].GetLocal ();
        //NS_LOG_DEBUG("sender is"<<sender );
...
void
RoutingProtocol::UpdateRouteToNeighbor (Ipv4Address sender, Ipv4Address receiver, Vector Pos, Vector f_Pos)
{
        m_neighbors.AddEntry (sender, Pos, f_Pos);
}
...
void RoutingProtocol::NotifyAddAddress (uint32_t interface, Ipv4InterfaceAddress address)
{
        ...
        {
                NS_LOG_LOGIC ("PGPSR does not work with more then one address per each interface. Ignore added address");
        }
}
//删除出地址
void
RoutingProtocol::NotifyRemoveAddress (uint32_t i, Ipv4InterfaceAddress address)
{
        ...
        {
                NS_LOG_LOGIC ("Remove address not participating in PGPSR operation");
        }
}
...
RoutingProtocol::SendHello ()
{
        ...
        f_positionX=positionX+0.5*velocityX;
        f_positionY=positionY+0.5*velocityY;
        ...
        for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j = m_socketAddresses.begin (); j != m_socketAddresses.end (); ++j)
        {
                Ptr<Socket> socket = j->first;
                Ipv4InterfaceAddress iface = j->second;
                HelloHeader helloHeader (((uint64_t) positionX),((uint64_t) positionY),((uint64_t) f_positionX),((uint64_t) f_positionY));
...
RoutingProtocol::LoopbackRoute (const Ipv4Header & hdr, Ptr<NetDevice> oif)
{
        ...
        NS_ASSERT_MSG (rt->GetSource () != Ipv4Address (), "Valid PGPSR source address not found");
                             ...
                             UnicastForwardCallback ucb, ErrorCallback ecb)
{
        ...
        if (nextHop != Ipv4Address::GetZero ())
        {
                ...
                double time_now = (double)Simulator::Now ().GetSeconds ();
                NS_LOG_DEBUG ("Now time is "<<time_now);
                NS_LOG_DEBUG ("In Forwarding Nexthop is  "<<nextHop);
