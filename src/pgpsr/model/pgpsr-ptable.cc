#include "pgpsr-ptable.h"
#include "ns3/simulator.h"
#include "ns3/log.h"
#include <algorithm>
#include <cmath>

NS_LOG_COMPONENT_DEFINE ("PGpsrTable");
...
{
        std::map<Ipv4Address, Metrix >::iterator i = m_table.find (id);
        //std::cout << "未来位置" << i->second.position.x << std::endl;
        ...
        {
				//std::cout << "IPアドレス" << i->first << std::endl;
				//std::cout << "未来位置" << i->second.position.x << std::endl;
                m_table.erase (id);
                Metrix metrix;
...
/**
 * \brief Gets next hop according to PGPSR protocol
...
PositionTable::BestNeighbor (Vector position, Vector nodePos)
{
	...
	NS_LOG_DEBUG ("My position is  " <<" (" <<nodePos.x<<","<<nodePos.y<<")");
      ...
      std::map<Ipv4Address, Metrix >::iterator i;;
	  double bestFoundDistance=10000000000; //
	  double R = 200;
      ...
      for (i = m_table.begin (); !(i == m_table.end ()); i++)
       {
         ...
         double distance = CalculateDistance(position,i->second.f_position);
         NS_LOG_DEBUG ("ID is  " <<i->first );
         //NS_LOG_DEBUG ("judge is  " <<judge );
         NS_LOG_DEBUG ("distance is  " <<distance );
         
         if(judge <= R*R && distance <= bestFoundDistance){
        ...
        if(bestFoundDistance<10000000000){
			//std::cout << bestFoundID <<  std::endl;
			NS_LOG_DEBUG ("return BestFoound ID is  " <<bestFoundID );
			NS_LOG_DEBUG ("");
			return bestFoundID;
        }
        else{
...
/**
 * \brief Gets next hop according to PGPSR recovery-mode protocol (right hand rule)
