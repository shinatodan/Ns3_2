...
public:
  /// c-tor
  HelloHeader (uint64_t originPosx = 0, uint64_t originPosy = 0 , uint64_t f_originPosx = 0, uint64_t f_originPosy = 0);
  ...
  //追加
   void SetOriginFPosx (uint64_t f_posx)
  {
    f_originPosx = f_posx;
  }
  uint64_t GetOriginFPosx () const
  {	
    return f_originPosx;
  }
  void SetOriginFPosy (uint64_t f_posy)
  {
    f_originPosy = f_posy;
  }
  uint64_t GetOriginFPosy () const
  {
    return f_originPosy;
  }//追加終了
  //\}
...
private:
  ...
  uint64_t         f_originPosx;          ///< Originator Position x
  uint64_t         f_originPosy;          ///< Originator Position x
};
